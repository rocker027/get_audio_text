...
`;
  ...
  async function handleTranscribe() {
    // 先檢查模型安裝狀態
    const modelInstalled = await new Promise((resolve) => {
      try {
        chrome.storage && chrome.storage.local.get(['whisper_model_installed'], (result) => {
          resolve(!!result['whisper_model_installed']);
        });
      } catch (e) {
        resolve(false);
      }
    ...
    if (!modelInstalled) {
      showNotification('請先在擴充套件彈窗下載安裝 Whisper 模型', 'error', 5000);
      return;
    }
    const videoInfo = {
    ...
    if (!videoInfo.url) {
      showNotification('無法偵測到影片資訊', 'error');
      return;
    }
    showNotification('🚀 開始音訊轉錄處理...', 'info');
    try {
      // 1. 擷取影片音訊
      const audioBlob = await getAudioBlobFromVideo();
      showNotification('音訊擷取完成，開始語音辨識...', 'info');
      // 2. 呼叫 Whisper worker 進行語音辨識
      const transcript = await transcribeWithWhisper(audioBlob);
      hideProgress();
      // 3. 下載逐字稿
      downloadTextFile(formatTranscript(videoInfo, transcript));
      showNotification('轉錄完成，已下載逐字稿', 'success');
    } catch (error) {
  ...
  async function getAudioBlobFromVideo() {
  ...
  async function transcribeWithWhisper(audioBlob) {
    // 讀取 worker 程式碼與模型
    const workerCode = await idbGet('whisper_worker_js');
    const modelBuf = await idbGet('whisper_model_bin');
    if (!workerCode || !modelBuf) throw new Error('Whisper 模型或 worker 未安裝');
    // 動態建立 worker
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.onmessage = (e) => {
        ...
        else reject(e.data.error || 'Whisper worker 發生錯誤');
      };
  ...
  function formatTranscript(videoInfo, transcript) {
    return `音訊轉錄結果\n========================\n\n影片標題: ${videoInfo.title}\n來源網址: ${videoInfo.url}\n轉錄時間: ${(new Date()).toLocaleString()}\n處理方式: Get Audio Text Chrome Extension\n\n轉錄內容:\n------------------------\n${transcript}\n\n========================\nGenerated by Get Audio Text Extension v1.0.0\n`;
  }
